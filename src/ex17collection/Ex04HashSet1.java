...
-인스턴스가 순서없이 저장된다
-중복저장을 허용하지 않는다
-기본클래스(String, Integer등)인 경우에는 별도의 처리없이
	중복이 제거된다
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		int vatInt = 100;//저장시 Integer로 Boxing 처리됨
		...
		/*
		기본클래스의 객체 중복저장
		: 기본 클래스의 인스턴스인 경우 별도의 오버라이딩 없이도
		...
		while(itr.hasNext()) {
			...
			/*
			HashSet에 저장시 모두 Object형으로 자동형변환되어
			저장되므로 우리가 직접 정의한 클래스에 대해서만
			instanceof를 통해 타입을 확인한 후
			다운캐스팅하여 출력한다.
			만약 다운캐스팅을 하지 않으면 자식멤버에 접근할 수
			없으므로 getName을 호출할 수 없다.
			 */
		...
		/*
		포함여부 확인 : set에 해당 인스턴스가 저장되어 있는지
			확인하여 boolean을 반환한다
		 */
