...
public class E05Date {
	...
	public static String myDateFormat(
		...
		/*
		서버의 표준시인 1970-01-01 부터 현재까지의 시간을
		초 단위로 반환해주는 메서드로 이를 '타임스탬프'라고 한다
		 */
		Date date = c.getTime();
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			서로 다르다. 따라서 각 날짜의 서식을 명확히 지정해야만
			날짜를 인식하여 연산할 수 있다.
			 */
			...
			밀리세컨즈를 의미한다. 즉 100으로 나눈 후
			뒤의 3자리를 제거하면 순순한 초가 반환된다.
			또한 하루는 60*60*24 즉 86400초이므로 아래와같이
			남은 날짜를 계산할 수 있다.
			 */
